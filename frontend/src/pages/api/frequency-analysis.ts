...
const debugLog = (context: string, data?: unknown) => {
  console.log(`[API:FrequencyAnalysis:${context}]`, data);
};
...
export default async function handler(
  ...
  try {
    ...
    const response = await fetch(
      `${BACKEND_URL}/api/frequency-analysis?userId=${userId}`,
      {
        method: "GET",
        headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(data.error || "Failed to fetch frequency analysis data");
    }
